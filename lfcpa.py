...
def perform_lfcpa(struct_dict, var_dict, stmt_lst):
    ...
    iters = []
    while True:
        l_iter = preform_ptbla(updt_stmt_list, successors, ptr_dicts, liveness_dicts)
        if l_iter == 1:
            iters.append((1,0))
            break
        pt_iter = preform_lbpta(updt_stmt_list, predecessors, ptr_dicts, liveness_dicts)
        iters.append((l_iter, pt_iter))
        if pt_iter == 1:
            break
    print("LFCPA Iteration -", len(iters), iters)
    get_points_to_graph(ptr_dicts[len(updt_stmt_list)-1], 'lfcpa')


def preform_lbpta(new_stmt_lst, predecessors, ptr_dicts, liveness_dicts):
    iter = 0
    change = True
    while change:
        change = False
...
def preform_ptbla(new_stmt_lst, successors, ptr_dicts, liveness_dicts):
    iter = 0
    change = True
